...
$class_wrap = array( 'yith_magnifier_gallery' );
$attr_wrap = array();

if( $enable_slider ){
    $attr_wrap[]    = 'data-dot="false"';
    $attr_wrap[]    = 'data-nav="true"';
    $attr_wrap[]    = 'data-owl="slide"';
    $attr_wrap[]    = 'data-item-slide="' . $columns . '"';
    $class_wrap[]   = 'owl-middle';
}

$attr_wrap[] = 'class="' . esc_attr( implode( ' ', $class_wrap ) ) . '"';

if ( $attachment_ids ) {
    ?>
    <div class="thumbnails <?php echo ( $enable_slider ) ? 'owl-container' : ''; ?>">
        <ul <?php echo implode( ' ', $attr_wrap ); ?>>
        <?php
        $loop = 0;
        ...
        foreach ( $attachment_ids as $attachment_id ) {
            ...
            echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<li class="%s"><a href="%s" class="%s" title="%s" data-small="%s">%s</a></li>', $image_class, $magnifier_url, $image_class, $image_title, $thumbnail_url, $image ), $attachment_id, $post_id, $image_class );
