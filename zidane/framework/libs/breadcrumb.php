...
class Inspius_Breadcrumb {
	...
	public function __construct(){
		$this->func = Inspius_Functions::instance();
	}
	...
	public function get_breadcrumb() {
		return apply_filters( 'is_get_breadcrumb', $this->crumbs, $this );
	}
	...
	public function generate() {
		...
		if( $this->func->is_request( 'woocommerce' ) ){
	...
	private function add_crumbs_404() {
		$this->add_crumb( esc_html__( 'Error 404', 'zidane' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'product' === get_post_type( $post ) ) {
			...
			if ( $terms = wc_get_product_terms( $post->ID, 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
				$main_term = apply_filters( 'is_breadcrumb_main_term', $terms[0], $terms );
	...
	private function add_crumbs_product_tag() {
		...
		$this->prepend_shop_page();
		$this->add_crumb( sprintf( esc_html__( 'Products tagged &ldquo;%s&rdquo;', 'zidane' ), $current_term->name ) );
	}
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Posts tagged &ldquo;%s&rdquo;', 'zidane' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Tác giả: %s', 'zidane' ), $userdata->display_name ) );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( esc_html__( 'Kết quả tìm kiếm cho &ldquo;%s&rdquo;', 'zidane' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( esc_html__( 'Page %d', 'zidane' ), get_query_var( 'paged' ) ) );
		}
	}
}
